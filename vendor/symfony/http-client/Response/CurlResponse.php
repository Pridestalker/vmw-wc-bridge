...
final class CurlResponse implements ResponseInterface
{
    ...
    public function __construct(CurlClientState $multi, $ch, array $options = null, LoggerInterface $logger = null, string $method = 'GET', callable $resolveRedirect = null)
    {
        ...
        curl_setopt(
            $ch, CURLOPT_HEADERFUNCTION, static function ($ch, string $data) use (&$info, &$headers, $options, $multi, $id, &$location, $resolveRedirect, $logger): int {
        ...
        if (null === $options) {
            ...
            curl_setopt(
                $ch, CURLOPT_WRITEFUNCTION, static function ($ch, string $data) use (&$content): int {
        ...
        if ($onProgress = $options['on_progress']) {
            ...
            curl_setopt(
                $ch, CURLOPT_PROGRESSFUNCTION, static function ($ch, $dlSize, $dlNow) use ($onProgress, &$info, $url, $multi, $debugBuffer) {
                    try {
                        rewind($debugBuffer);
        ...
        curl_setopt(
            $ch, CURLOPT_WRITEFUNCTION, static function ($ch, string $data) use (&$content, $multi, $id): int {
